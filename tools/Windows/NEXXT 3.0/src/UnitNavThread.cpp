		...
		for(i=0;i<minH;i++)
		{
			...
			for(j=0;j<minW;j++)
			{
				if(j>=0&&j<w&&i>=0&&i<h)
				{
					...
					highlight_tile=nameTable[((i*w)/(w*cgY))*(w*cgY) + (j/cgY)*cgX];
					...
					else
					{
						...
						{
							FormMain->DrawTile(FormNavigator->Map1->Picture,x,y,tile,highlight_tile,FormMain->AttrGet(j,i,false,false),j,i,false,false,1,true,false,bHalfsize,false,false);
						}
						else
						{
