...
def main():
  fulldrawing = Image.open("../chr/raw/mario_angry.bmp")
  ...
  for i in range(0,angles):
    ...
    outbig = Image.new("P", (128, 32))
    outbig.putpalette(fulldrawing.palette)
    ...
    out.save(f"../build/rotate_{i}.bmp")
    b = pilbmp2chr(out, 8, 16)
    with open(f"../chr/rotate/small_rotate_{i}.chr", 'wb') as f:
      f.write(b''.join(b))
    with open(f"../chr/rotate/big_rotate_{i}.chr", 'wb') as f:
      f.write(b''.join(b))
